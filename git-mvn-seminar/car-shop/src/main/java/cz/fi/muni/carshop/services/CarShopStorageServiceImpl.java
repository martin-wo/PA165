...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void addCarToStorage(Car car) {
		if (car.getPrice() < 0)
			throw new IllegalArgumentException("Car price must not be less than 0.");
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
		...
		if (carsOfSameType == null || !carsOfSameType.remove(car))
			throw new RequestedCarNotFoundException("The requested car does not exist in storage");
	}
